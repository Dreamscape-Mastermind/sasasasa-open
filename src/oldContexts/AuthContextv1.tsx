...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const handleRouteCheck = () => {
      ...
      if (category === "PROTECTED") {
        ...
        if (
          path.startsWith(ROUTES.DASHBOARD_EVENT_DETAILS("")) &&
          !hasAnyAccessLevel([
            ACCESS_LEVELS.EVENT_ORGANIZER,
            ACCESS_LEVELS.EVENT_TEAM,
          ])
        ) {
          router.push(ROUTES.UNAUTHORIZED);
          return;
    ...
    const initializeAuth = async () => {
      ...
      try {
        ...
        if (storedUser && storedTokens) {
          if (tokenService.validateToken(storedTokens.access)) {
            setUser(storedUser);
  ...
  const logout = async () => {
    if (tokens) {
      try {
        await logoutMutation.mutateAsync(tokens.refresh);
  ...
  const getAccessToken = useCallback((): string | null => {
    ...
    return tokenService.validateToken(storedTokens.access)
      ? storedTokens.access
      : null;
  }, []);
