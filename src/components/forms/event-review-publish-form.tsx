...
export default function EventReviewPublishForm({
  ...
  const handleUnpublish = async () => {
    ...
    setTopError(null);
    try {
      await unpublishEventMutation.mutateAsync(eventId);
      toast.success("Event unpublished successfully!");
      await refetchEvent();
    } catch (error) {
      let fallbackMessage = "Failed to unpublish event";
      if (error instanceof ApiError) {
        const apiMessage = error.data?.message || error.message;
        const detail = error.data?.result?.errors?.error_details?.detail;
        setTopError(detail || apiMessage || fallbackMessage);
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      let fallbackMessage = "Failed to cancel event";
      if (error instanceof ApiError) {
        const apiMessage = error.data?.message || error.message;
        const detail = error.data?.result?.errors?.error_details?.detail;
        setTopError(detail || apiMessage || fallbackMessage);
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-background/95 text-foreground">
      <div className="max-w-7xl mx-auto px-3 sm:px-6 py-4 sm:py-8">
        ...
        <div className="max-w-4xl mx-auto space-y-6 sm:space-y-8">
              ...
              <div className="flex items-center gap-3 sm:gap-4">
                ...
                <div>
                  ...
                  <CardDescription className="text-xs sm:text-sm text-muted-foreground">
                    Review all the information you've entered before publishing
                    your event
                  </CardDescription>
                </div>
              </div>
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="space-y-1">
                            ...
                            {(event.minimum_age || event.maximum_age) && (
                              <p className="text-sm text-muted-foreground">
                                {event.minimum_age &&
                                  `Min: ${event.minimum_age}`}
                                {event.minimum_age &&
                                  event.maximum_age &&
                                  " • "}
                                {event.maximum_age &&
                                  `Max: ${event.maximum_age}`}
                              </p>
                            )}
                          </div>
                        </div>
                ...
                <>
                  <div className="space-y-2">
                    ...
                    <div className="relative w-full max-w-md h-48 rounded-lg overflow-hidden border">
                      <NextImage
                        src={
                          event.cover_image || "/images/placeholder-event.jpg"
                        }
                        alt="Event cover"
                        width={400}
                        height={192}
          ...
          {/* Action Buttons */}
          <div className="space-y-4 pt-6 sm:pt-8 border-t border-gray-200 dark:border-gray-700">
            ...
            <div className="flex flex-wrap gap-2 justify-center sm:justify-end">
              {event.status === EventStatus.PUBLISHED ? (
                <ConfirmationDialog
                  ...
                  isLoading={unpublishEventMutation.isPending}
                />
              ) : (
                <ConfirmationDialog
                  trigger={
