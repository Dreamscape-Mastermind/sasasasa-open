...
import { useCreateQueryString } from '@/utils/utils';
...
function EmailSettingsContent() {
  ...
  const handleLinkEmail = async (e: React.FormEvent) => {
    ...
    setIsLinking(true);
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_SASASASA_API_URL}api/v1/web3/add_email`, {
        method: 'POST',
        headers: {
  ...
  const handleVerifyEmail = async (e: React.FormEvent) => {
    ...
    setIsVerifying(true);
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_SASASASA_API_URL}api/v1/web3/verify_email`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${getAccessToken()}`
