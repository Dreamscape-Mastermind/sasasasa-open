  ...
  return (token: string): boolean => {
    ...
    try {
      ...
      if (process.env.NODE_ENV === "development") {
        ...
        console.log(
          `Token expires in: ${Math.floor(timeLeft / 60000)}m ${Math.floor(
            (timeLeft % 60000) / 1000
          )}s`
        );
      }
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const logout = async () => {
    if (tokens) {
      await fetch(
        `${process.env.NEXT_PUBLIC_SASASASA_API_URL}api/v1/accounts/logout`,
        {
          method: "POST",
          headers: {
  ...
  const getAccessToken = useCallback((): string | null => {
    ...
    const parsedTokens: AuthTokens = JSON.parse(tokens);
    return memoizedTokenValidation(parsedTokens.access)
      ? parsedTokens.access
      : null;
  }, []);
  ...
  const loginWithSIWE = async (address: `0x${string}`) => {
    ...
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_SASASASA_API_URL}api/v1/web3/nonce`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_SASASASA_API_URL}api/v1/web3/verify`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  const refreshAccessToken = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_SASASASA_API_URL}api/v1/accounts/refresh-token`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const loginWithSIWEReCap = async (
    ...
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_SASASASA_API_URL}api/v1/web3/recap_nonce`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_SASASASA_API_URL}api/v1/web3/verify_recap`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const linkWallet = async (address: `0x${string}`) => {
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_SASASASA_API_URL}api/v1/web3/link_wallet`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      const verifyRes = await fetch(
        `${process.env.NEXT_PUBLIC_SASASASA_API_URL}api/v1/web3/verify_link_wallet`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
