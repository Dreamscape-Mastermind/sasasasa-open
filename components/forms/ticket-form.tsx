...
import { CalendarIcon, Circle, ImagePlus, Loader2, MapPin, Check, CircleDot, X } from 'lucide-react'
...
import { useCreateTicket, useUpdateTicket, useDeleteTicket } from "services/tickets/mutation"
  ...
  const onSubmitTicket = async (data: z.infer<typeof ticketSchema>) => {
    try {
      ...
      if (editingTicketId && eventId) {
        await updateTicket.mutateAsync({ eventId, ticketId: editingTicketId, data: processedTicket });
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>All tickets created for this event</CardDescription>
          </CardHeader>
          <CardContent>
                ...
                tickets?.result?.results?.map((ticket) => (
                  <Card key={ticket.id} className="p-4">
                    ...
                    <CardContent>
                      ...
                      <p>
                        Sale Period: {format(new Date(ticket.sale_start_date), "PPP")} -
                        {format(new Date(ticket.sale_end_date), "PPP")}
                      </p>
                    </CardContent>
                    <CardContent>
        ...
        <Card ref={formRef}>
          <CardHeader>
            ...
            <CardDescription>Create a new ticket type for your event</CardDescription>
          </CardHeader>
          <CardContent>
            <Form {...ticketForm}>
              <form onSubmit={ticketForm.handleSubmit(onSubmitTicket)} className="space-y-6">
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input className="rounded-full" placeholder="e.g., VIP Ticket, Early Bird" {...field} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                ...
                >
                  {isEditing ? 'Edit Ticket' : (createTicket.isPending ? (
                    <>
