...
import { useInviteTeamMember } from "services/events-team/mutations"
import { useSearchParams } from "next/navigation"
import { usePublishEvent } from "services/events/mutation"; // Import the usePublish hook
...
function TeamMembersFormContent() {
  ...
  const inviteTeamMember = useInviteTeamMember(); // Use the mutation to invite a team member

  const handleInvite = async () => {
    const memberData = { user_email: teamForm.getValues("user_email"), role: teamForm.getValues("role") }; // Get email and role from form
    if (eventId) await inviteTeamMember.mutateAsync({ eventId, data: memberData });
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-zinc-900 text-gray-900 dark:text-gray-200 p-6">
      <div className="max-w-6xl mx-auto">
        ...
        <Card className="mb-6">
          <CardHeader>
            <CardTitle className="text-lg font-bold">Current Team Members</CardTitle>
          </CardHeader>
          <CardContent>
            ...
            <Form {...teamForm}>
              <form onSubmit={teamForm.handleSubmit(handleInvite)} className="flex items-center space-x-4 mt-4">
                ...
                />
                <Button type="submit" variant="outline" size="sm" className=" dark:bg-zinc-900 dark:border-gray-700 rounded-full">
                  Invite Team Member
                </Button>
              </form>
            </Form>
